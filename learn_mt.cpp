...
private:
    mutable std::vector<std::mt19937> thread_rngs;
...
public:
    GeneticAlgorithm(int pop_size = 100, float mut_rate = 0.2f, float mut_strength = 0.2f)
        ...
        for (int i = 0; i < num_threads; i++) {
            thread_rngs[i].seed(std::chrono::steady_clock::now().time_since_epoch().count() + i);
        }
    ...
    // Parallelized evaluate_fitness method
    float evaluate_fitness(const NeuralNetwork& network, int games = 5) {
        // If only 1 game, don't bother with threading overhead
        if (games == 1) {
            return evaluate_single_game(network);
        }
        
        // Launch games in parallel
        std::vector<std::future<float>> game_futures;
        game_futures.reserve(games);
        
        // Lambda function to play a single game
        auto play_single_game = [this, &network]() -> float {
            return evaluate_single_game(network);
        };
        
        // Launch all games asynchronously
        for (int i = 0; i < games; i++) {
            game_futures.push_back(std::async(std::launch::async, play_single_game));
        }
        ...
        for (auto& future : game_futures) {
            float game_result = future.get();
            total_score += game_result;
            
            // Extract some stats for bonus calculations
            if (game_result > 1000.0f) {  // Rough threshold for successful games
                successful_games++;
            }
        }
        ...
        return avg_fitness + consistency_bonus;
    }

    // Helper method to evaluate a single game
    float evaluate_single_game(const NeuralNetwork& network) {
        Game game;
        int steps = 0;
        int food_eaten = 0;
        ...
        while (!game.is_game_over() && steps < 2500) {
            ...
            for (int i = std::max(0, (int)position_history.size() - recent_limit); i < (int)position_history.size(); i++) {
        ...
        return std::max(game_fitness, -1000.0f);
    }
    ...
    void evolve(bool verbose = false) {
        ...
        auto evaluate_batch = [this](int start_idx, int end_idx) {
            for (int i = start_idx; i < end_idx; i++) {
                fitness_scores[i] = evaluate_fitness(*population[i]);
            }
        };
        
        int batch_size = population_size / num_threads;
        for (int t = 0; t < num_threads; t++) {
            int start = t * batch_size;
            int end = (t == num_threads - 1) ? population_size : (t + 1) * batch_size;
            
            futures.push_back(std::async(std::launch::async, evaluate_batch, start, end));
        }
